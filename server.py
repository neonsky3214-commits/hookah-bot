    ...
    try:
        ...
        return web.json_response({"bookings": [
            {"id": r["id"], "zone": r["zone"], "table_num": r["table_num"],
             ...
             "guests": r["guests"], "name": r["name"], "status": r["status"], "comment": r.get("comment") or ""}
            for r in rows
        ]})
    except Exception as e:
        return web.json_response({"bookings": []})
...
async def api_reschedule_booking(request):
    try:
        data = await request.json()
        bid = int(data.get("booking_id") or 0)
        tg_user_id = int(data.get("tg_user_id") or 0)
        new_date = str(data.get("new_date") or "")
        new_time = str(data.get("new_time") or "")
        if not bid or not tg_user_id or not new_date or not new_time:
            return web.json_response({"ok": False, "error": "Missing fields"})
        async with db_pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT * FROM bookings WHERE id=$1 AND tg_user_id=$2 AND status='active'",
                bid, tg_user_id
            )
            if not row:
                return web.json_response({"ok": False, "error": "Бронь не найдена или уже отменена"})
            await conn.execute(
                "UPDATE bookings SET book_date=$1, book_time=$2, reminder_sent=FALSE WHERE id=$3",
...
async def api_booking_comment(request):
    try:
        data = await request.json()
        bid = int(data.get("booking_id") or 0)
        tg_user_id = int(data.get("tg_user_id") or 0)
        comment = str(data.get("comment") or "").strip()[:500]
        if not bid or not tg_user_id:
            return web.json_response({"ok": False})
        async with db_pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT id FROM bookings WHERE id=$1 AND tg_user_id=$2", bid, tg_user_id
            )
            if not row:
                return web.json_response({"ok": False, "error": "Not found"})
            await conn.execute("UPDATE bookings SET comment=$1 WHERE id=$2", comment, bid)
        return web.json_response({"ok": True})
    except Exception as e:
        return web.json_response({"ok": False, "error": str(e)})
